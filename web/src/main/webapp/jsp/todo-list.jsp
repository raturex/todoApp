...
<section id="tasker">
    <header id="header">
        ...
        <form action="<c:url value="insert"/>" method="POST">
            ...
            <input id="new-task" name="name" placeholder="What needs to be done?" autofocus>
        </form>
    </header>
    <section id="main">
        ...
        <ul id="task-list">
            <c:forEach var="taskItem" items="${taskItems}" varStatus="status">
                <li id="taskItem_${status.count}" class="<c:if test="${taskItem.completed}">completed</c:if>" ondblclick="javascript:document.getElementById('taskItem_${status.count}').className += ' editing';document.getElementById('taskItemName_${status.count}').focus();">
                    <div class="view">
                        <form id="toggleForm_${status.count}" action="<c:url value="toggleStatus"/>" method="POST">
                            <input type="hidden" name="id" value="${taskItem.id}"/>
                            <input type="hidden" name="filter" value="${filter}"/>
                            <input class="toggle" name="toggle" type="checkbox" <c:if test="${taskItem.completed}">checked</c:if> onchange="javascript:document.getElementById('toggleForm_${status.count}').submit();">
                        </form>
                        <label>${taskItem.description}</label>
                    ...
                    <form id="updateForm_${status.count}" action="<c:url value="update"/>" method="POST">
                        <input type="hidden" name="id" value="${taskItem.id}"/>
                        <input type="hidden" name="filter" value="${filter}"/>
                        <input class="edit" id="taskItemName_${status.count}" name="name" value="${taskItem.description}" onblur="javascript:document.getElementById('updateForm_${status.count}').submit();"/>
                    </form>
                </li>
            </c:forEach>
        </ul>
    ...
    <footer id="footer">
        <c:if test="${stats.all > 0}">
            <span id="task-count"><strong><c:out value="${stats.active}" /></strong>
            <c:choose>
